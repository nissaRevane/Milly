...
RSpec.describe BalanceSheet, type: :model do
  ...
  describe ".liabilities_breakdown_for" do
    ...
    it "counts the autres crédits among the dettes diverses" do
      sheet = create(:balance_sheet, user: user)
      credit = create(:liability, user: user, liability_type: :other_credit)
      debt = create(:liability, user: user, liability_type: :short_term_debt)
      deposit = create(:liability, user: user, liability_type: :security_deposit)
      create(:balance_sheet_liability, balance_sheet: sheet, liability: credit, remaining_capital: 8_000)
      create(:balance_sheet_liability, balance_sheet: sheet, liability: debt, remaining_capital: 2_000)
      create(:balance_sheet_liability, balance_sheet: sheet, liability: deposit, remaining_capital: 1_200)

      series = BalanceSheet.liabilities_breakdown_for([sheet])

      expect(series.map(&:key)).to eq(%w[short_term_debt security_deposit])
      expect(series.first.full_label).to eq("Dettes diverses")
      expect(series.first.values).to eq([10_000])
    end
